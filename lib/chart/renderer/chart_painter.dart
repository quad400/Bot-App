...
import '../model/flutter_k_chart.dart';

//adjustable trend line
double? trendLineX;
...
class ChartPainter extends BaseChartPainter {
  final List<TrendLine> trendLines; //For TrendLine
  ...
  ChartPainter(
    ...
    this.chartColors,
    this.indicatorColor, {
    required this.trendLines, //For TrendLine
  ...
  void initChartRenderer() {
    ...
    if (mVolRect != null) {
      mVolRenderer = VolRenderer(mVolRect!, mVolMaxValue, mVolMinValue,
          mChildPadding, fixedLength, chartStyle, chartColors);
    }
    if (mSecondaryRect != null) {
      mSecondaryRenderer = SecondaryRenderer(
          ...
          fixedLength,
          chartStyle,
          chartColors, indicatorColor);
    }
  }

// full chart screen decoration
  @override
  void drawBg(Canvas canvas, Size size) {
    ...
    Gradient mBgGradient = LinearGradient(
      ...
      colors: chartColors.bgBlackColor,
    );
    Rect mainRect =
...
// full chart screen grid plot
  ...
  void drawMaxAndMin(Canvas canvas) {
    ...
    if (x < mWidth / 2) {
      TextPainter tp = getTextPainter(
          '______ ${mMainLowMinValue.toStringAsFixed(fixedLength)}',
          chartColors.minColor);
      tp.paint(canvas, Offset(x, y - tp.height / 2));
    } else {
      TextPainter tp = getTextPainter(
          '${mMainLowMinValue.toStringAsFixed(fixedLength)} _____',
          chartColors.minColor);
    ...
    if (x < mWidth / 2) {
      TextPainter tp = getTextPainter(
          '______ ${mMainHighMaxValue.toStringAsFixed(fixedLength)}',
          chartColors.maxColor);
      tp.paint(canvas, Offset(x, y - tp.height / 2));
    } else {
      TextPainter tp = getTextPainter(
          '${mMainHighMaxValue.toStringAsFixed(fixedLength)} _____',
          chartColors.maxColor);
      tp.paint(canvas, Offset(x - tp.width, y - tp.height / 2));
    }
  }
  ...
  void drawNowPrice(Canvas canvas) {
    ...
    TextPainter tp = getTextPainter(
        value.toStringAsFixed(fixedLength), chartColors.nowPriceTextColor);
