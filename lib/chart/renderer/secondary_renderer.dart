...
import '../model/k_chart_widget.dart' show SecondaryState;
...
class SecondaryRenderer extends BaseChartRenderer<MACDEntity> {
    ...
    switch (state) {
      ...
      case SecondaryState.CCI:
        drawLine(lastPoint.cci, curPoint.cci, canvas, lastX, curX,
            indicatorColor.rsiColor);
        break;
      case SecondaryState.STD:
        drawStandardDeviation(lastPoint, curPoint, canvas, lastX, curX);
        break;
      default:
        break;
    }
  ...
  void drawStandardDeviation(MACDEntity lastPoint, MACDEntity curPoint,
      Canvas canvas, double lastX, double curX) {
    if (lastPoint.standardDeviation != 0) {
      drawLine(lastPoint.standardDeviation, curPoint.standardDeviation, canvas,
          lastX, curX, chartColors.upColor);
    }
  }
    ...
    switch (state) {
      ...
      case SecondaryState.RSI:
        children = [
          TextSpan(
              text:
                  'RSI(${IndicatorInput.rsiControllerPeriod.text}): ${format(data.rsi)}    ',
              style: getTextStyle(indicatorColor.rsiColor)),
        ];
        break;
      case SecondaryState.WR:
        children = [
